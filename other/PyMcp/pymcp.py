...
class mcp1D():
	...
	def RawMcpToMantid(self,name):
		...
		for f in (self.detectors):
			CreateWorkspace(name+'Det_'+str(f),self.pz,self.datraw[f-1],self.rawerr[f-1])
				
		for f in (self.detectors):
			ConjoinWorkspaces(InputWorkspace1=name+'Det_'+str(1),InputWorkspace2=name+'Det_'+str(f),CheckOverlapping="0")
...
class mcp():
	...
	def RawDataToMantid(self,name):
		...
		for f in (self.detectors):
			ConjoinWorkspaces(InputWorkspace1=name+'_Updata_Det_'+str(1),InputWorkspace2=name+'_Updata_Det_'+str(f),CheckOverlapping="0")
		
		for f in (self.detectors):
			CreateWorkspace(name+'_Dndata_Det_'+str(f),self.ch[f-1],np.nan_to_num(self.dndata[f-1]),np.nan_to_num(self.dnerr[f-1]))
				
		for f in (self.detectors):
			ConjoinWorkspaces(InputWorkspace1=name+'_Dndata_Det_'+str(1),InputWorkspace2=name+'_Dndata_Det_'+str(f),CheckOverlapping="0")
