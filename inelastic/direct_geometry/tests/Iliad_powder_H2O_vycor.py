...
wb=5545    # enter whitebeam run number here
#run_no=range(5999,6002) #e0nr event mode run numbers here or use next line for a continous sequence of runs i.e range(first run, last run +1)
run_no=range(6025,6069)#[6020]
ei=[20,6,2.83,1.64]           # incident energies you want analysed, or leave as ei=[]  if you want all incident energies analysed
#ei=[3.5]
ebin=[-1,0.002,0.95]    #binning of the energy for the spe file. The numbers are as a fraction of ei [from ,step, to ]
...
#mapping='central_strip' 
mapping='rings_103' # one to one mapping of detectors for crystals
file = '/Users/jon/Work-computing/mantid_test_data/let/hard.msk'    # standard hard mask file  for LET
#file = '/home/let/Desktop/LET_maps/magnet_hard.msk'  #9T mask
#file = '/home/let/Desktop/LET_maps/magnet7T_hard.msk'    # standard hard mask file  for LET
#file = '/home/let/Desktop/LET_maps/Mask_20deg_strip.msk'
############################################



####create list of spectra to mask out
#f=open(file,'r')
#s=f.readline()
...
for run in run_no:     #loop around runs
	...
	#LoadEventNexus(Filename='let0000'+str(run) +'.nxs',OutputWorkspace='w1',SingleBankPixelsOnly='0',LoadMonitors='1',MonitorsAsEvents='1',FilterByTimeStart='5400')
...
#	print (ei)
	...
	for energy in ei:
		...
		print ebinstring
		out=iliad("wb_wksp","w1reb",energy,ebinstring,mapping,bleed=False,norm_method='current',det_cal_file='det_corrected7.dat',detector_van_range=[0.5,200],bkgd_range=[int(t_elastic),int(tmax)],hardmaskOnly=file)
		#SaveNXSPE(out,'/home/let/Users/jon/LET'+str(run)+'_'+str(energy)+'mev.nxspe')
		#SaveSPE(out,'/home/let/Users/Chisnell/spe/LET'+str(run)+'_'+str(energy)+'mev.spe')
		RenameWorkspace(out,str(run)+'_'+str(energy)+'mev')
