...
path="\\\isis\inst$\NDXSANS2D\Instrument\data\cycle_12_4\\"
#path="U:\\processed\\"
pfix="sans2d"
sfix=".nxs"
#
#runlist=[13596,13630,13631,13632,13633,13634,13635]
#runlist=[13596,13644,13645,13646,13647,13648]
#runlist=[13596,13649,13650,13651,13652,13653]
#runlist=[16215,16216,16217,16218,16219,16220,16221,16223,16224,16225,16226,16227,16228,16229,16230,16231,16232,16233,16234,16235,16236,16237,16238,16239,16240,16241,16243,16244,16245,16246,16247]
runlist=[16398]
#
...
output_file.write('Run_number,M1,err,M2/M1_short,err,M2/M1_long,err,M4/M1_short,err,M4/M1_long,err\n')
#
#
# LOOP OVER RUNLIST ===================================================================================
...
#
	...
	numfiles=numfiles+1
	# this makes a workspace called "out" not very helpful
	#out=run+"_norm"
	#out=NormaliseByCurrent(run+"_norm")
	# Are we supposed to do this any more ?
...
#
	...
	CropWorkspace(run+"_norm", run+"_m4", StartWorkspaceIndex=monitor4, EndWorkspaceIndex=monitor4)
	ConvertUnits(run+"_m4",run+"_m4","Wavelength")
...
#
# FOURTH INTEGRATION ========================================================================================
	Integration(run+"_m4","integral",str(3.0),str(5.0))
#
	try:
...
#	
# FITH INTEGRATION ========================================================================================
	Integration(run+"_m4","integral",str(7.0),str(12.0))
	try:
...
CreateWorkspace("sums_M4_short",outlist,sumlist4,errsumlist4)
CreateWorkspace("sums_M4_long",outlist,sumlist5,errsumlist5)
#
...
for runnum in runlist:
	run=str(runnum)
	if runnum==runlist[0]:
		plot4=plotSpectrum(run+"_m4",0)
		layer=plot4.activeLayer()
		layer.setTitle("monitor 4")
	else:
		mergePlots(plot4,plotSpectrum(run+"_m4",0))
	layer.setCurveTitle(i,run)
	i=i+1
