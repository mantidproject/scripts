###########################################################################
#  14/8/9 attempt to get neutrons PER UAMPHR vs Q 
...
#and "TransmissionMonitor", with the defaults being the LOQ values so nothing should be broken that wasn't already!
	...
	dimension=192
# SKIP  ========  load up the SANS2d transmission runs
#	if trans_sample <> ' ':
#		LoadRaw(path + prefix + '00000'+str(trans_sample_integer) + ext, trans_sample, transmonitorspectrum,transdetspectrum)
#	if direct_sample <> ' ':
#		LoadRaw(path + prefix + '00000'+str(direct_sample_integer) + ext, direct_sample, transmonitorspectrum,transdetspectrum)
...
#	print sin(RotRadians)
	xshift = str( (Rear_Det_x + Rear_Det_x_corr - Front_Det_x - Front_Det_x_corr + Front_Det_Radius*sin(RotRadians ) )/1000. - front_det_default_x_m -xcentre)
	yshift = str( Front_Det_y_corr /1000.  - ycentre)
	# default in instrument description is 23.281m - 4.000m from sample at 19,281m !
	zshift = str( (Front_Det_z + Front_Det_z_corr + Front_Det_Radius*(1 - cos(RotRadians )) )/1000.  -front_det_default_sd_m)
...
#              mantid.deleteWorkspace(direct_tmp_out)
	...
	#mantid.deleteWorkspace(monitorWS)
 	...
 	if instr_name == 'skipSANS2D'and transWS and direct_sample:
		trans_ws = transWS.split('_')[0] + "_trans_" + suffix
		if usePreviousTrans == ' ':
...
wav1=2.0
wav2=12.0
final_workspace = scatter_sample.split('_')[0] + bank + "_2_12"
Correct(scatter_sample, trans_sample, final_workspace, "part")
#
wav1=4.0
wav2=12.0
final_workspace = scatter_sample.split('_')[0] + bank + "_4_12"
Correct(scatter_sample, trans_sample, final_workspace, "part")
wav1=6.0
wav2=12.0
final_workspace = scatter_sample.split('_')[0] + bank + "_6_12"
Correct(scatter_sample, trans_sample, final_workspace, "part")
wav1=8.0
wav2=12.0
final_workspace = scatter_sample.split('_')[0] + bank + "_8_12"
Correct(scatter_sample, trans_sample, final_workspace, "part")
wav1=10.0
wav2=12.0
...
wlist=["_1D","_2_12","_4_12","_6_12","_8_12","_10_12"]
