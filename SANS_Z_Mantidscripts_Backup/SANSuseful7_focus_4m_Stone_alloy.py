...
DataPath("N:/cycle_11_2/")
DataPath("U:/processed/")
...
maskfilename='U:/Masks/MASKSANS2D_112E_4m_Stone_furnace_RKH_TEST.txt'
# pull in the direct beam that we need to modify MAKE SURE IT IS THE SAME AS IN THE MASK FILE !
LoadRKH("U:/Masks/DIRECT_try_Stone1.dat","directbeam")
rsam="8095"
rcan="8092"
tsam="8094"
tdb="8091"
tcan="8093"
#wlist=[1.75,2.0,14.0,16.5]
# cannot get it to work from 1.875, its the default trans fit again!
wlist=[1.75,1.975,2.125,2.375,2.625,2.875,3.125,3.625,4.125,4.625,5.125,5.625,6.0,7.0,8.0,9.0,10.0,11.0,12.0,14.0,16.5]
...
TransFit('Log',lambdamin=1.75,lambdamax=16.5)
...
nloops=len(wlist)-1
reduced = WavRangeReduction(wlist[0], wlist[nloops], DefaultTrans)
Integration(reduced,"integral",Q1,Q2)
...
#  now integrate around Qmatch and ratio to the full range data - later could do proper fits here
	Integration(reduced,"integral",Q1,Q2)
	scaleX.append(wav1)
...
scaleY[0]=scaleY[2]*(0.75-0.5)/(wlist[1]-wlist[0])
scaleY[1]=scaleY[2]*(wlist[0]-0.75)/(wlist[1]-wlist[0])
# ======================================================================================
